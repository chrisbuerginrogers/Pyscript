...
async def wait(waitTime = 1):
    await asyncio.sleep(waitTime)

# pass along one or more callbacks to `init`
# so that the `xworker` can be sync'd and used

async def wait_channel(msg):
    while channel.msg != msg:
        await asyncio.sleep(0.1)
...
def george(cmd, array = []):
    try:
        if cmd == 'hubType':
            return motor.hubType
        elif cmd == 'reply':
            return sensor._hub.reply
        elif cmd == 'run':
            motor.run(*array) # speed=speed, port=port, direction=direction      
        elif cmd == 'stop':
            motor.stop(*array) # port=port       
        elif cmd == 'myspeed':
            motor.set_speed(*array)#speed=speed, port=port        
        elif cmd == 'run_angle':
            print('trying to angle')
            motor.run_angle(*array) #angle=angle, port = port, direction=direction        
        elif cmd == 'run_to':
            print('moving to position')
            motor.run_to(*array) # pos=position, port = port, direction=direction        
        else:
            return None   
    except Exception as e:
...
def becca(cmd):
    try:
        return json.dumps(sensor._hub.reply)
    except Exception as e:
        print('error: ',e)
    
window.init.resolve({"becca": becca, "george":george, "fred": fred, "wait": wait , "wait_channel": wait_channel })
