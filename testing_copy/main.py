...
def george(cmd, **kwargs):
    if cmd == 'hubType':
        return motor.hubType
    elif cmd == 'reply':
        return motor._hub.reply
    elif cmd == 'run':
        speed = kwargs.get('speed', 100)
        port = kwargs.get('port', 1)
        direction = kwargs.get('direction', 2)
        motor.run(**kwargs)#speed=speed, port=port, direction=direction)        
        return True
    else:
        return None   

window.init.resolve({"george":george, "fred": fred, "wait": wait , "wait_channel": wait_channel })
'''
def run(speed = 100, port = 1, direction = 2):
def myspeed(speed_value = 100, port = 1): 
def stop(port = 1):    
