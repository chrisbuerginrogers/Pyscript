...
class Hub_PS:
    ...
    async def ask(self, event): 
        if self.sync.innerHTML == 'Connect':
            ...
            try:
                ...
                window.console.log('trying again ',fmt, ID)
                await self.send(fmt, ID)
                for i in range(50):
                    if self.info:  # waits for the info_response to arrive and get parsed
                        await self.feed_rate(50)
                        return
                    print('waiting')
                    await asyncio.sleep(0.1)
                window.console.log('cannot find element')
    ...
    async def send(self, fmt, ID, val = None):
        window.console.log(fmt, ID, val)
        payload = [ID]
        if val:
            payload.extend(val)
        window.console.log('Values: ',payload)
